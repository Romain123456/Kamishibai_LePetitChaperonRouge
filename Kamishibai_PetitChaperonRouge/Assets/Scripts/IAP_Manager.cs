...
public class IAP_Manager : MonoBehaviour, IStoreListener
{
    ...
    void Start()
    {
        ...
        //Teste si l'utilisateur a acheté le produit
        CheckProduct(_BuyEntireKamishibai);
    }



    //Vérification d'achat du produit
    public void CheckProduct (string productID)
    {
#if UNITY_EDITOR
        //StartCoroutine(VerifyProduct(productID));
#elif UNITY_ANDROID
        StartCoroutine(VerifyProduct(productID));
#endif
    }

    IEnumerator VerifyProduct(string productID)
    {
        ...
        {
            if (productID == _BuyEntireKamishibai)
            {
                GameObject.Find("Main Camera").GetComponent<LivreManagement>().appliDemo = false;
            }
        } else
        {
            GameObject.Find("Main Camera").GetComponent<LivreManagement>().appliDemo = true;
        }
    }
